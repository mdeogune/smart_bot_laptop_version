...
os.chdir('/home/mukesh-deo/Documents/dataset_1/train_nn')
global x
global y
global z
global c_img
...
def location(img):
    ...
    left_y=0
    right_y=600
    for y1 in range(300,1,-1):
        if img[x,y1]==255:
            left_y=y1
            break

    #print left_y
    for y1 in range(300,600):
        if img[x,y1]==255:
            right_y=y1
            break
    pos=round(((left_y+right_y)/2))
    #pos=str(pos)
    return pos
...
def process_img(p_img):
    ...
    p_img=cv2.Canny(p_img,threshold1=180,threshold2=200)
    p_img=cv2.GaussianBlur(p_img,(21,21),1)
    ...
    lines=cv2.HoughLinesP(p_img,1,np.pi/180,180,np.array([]),50,0)
##    try:
...
##    cv2.cv.CreateTrackbar("Z axis", "undistorted", int(z),600, resetZ)
   ...
   # p_img=cv2.resize(p_img,(60,60))
    #frame1=cv2.resize(frame1,(60,60))
   # cv2.imwrite('img_'+str(i)+'.jpg',p_img)
    print('Loop took {} seconds'.format(time.time()-last_time))
    last_time=time.time();
    
    print location(p_img)
    arduino(location(p_img))
    
    #cv2.imwrite('_'+location(p_img)[0]+'.'+str(i)+'.jpg',frame1)
    if cv2.waitKey(1)& 0xFF == ord('q'):
        break
#cv2.imwrite('img_',p_img)
cv.WaitKey(0)
cap.release()
