...
def location(img):
    ...
    left_y=0
    right_y=60
    for y1 in range(30,1,-1):
        if img[x,y1]==255:
            left_y=y1
            break

    #print left_y
    for y1 in range(30,59):
        if img[x,y1]==255:
            right_y=y1
            break
...
def process_img(p_img):
    ...
    lines=cv2.HoughLinesP(p_img,1,np.pi/280,100,np.array([]),1,0)
##    try:
...
##    cv2.cv.CreateTrackbar("Z axis", "undistorted", int(z),600, resetZ)
    ...
    #frame=scipy.misc.imresize(frame, 30, interp='bilinear', mode=None)
